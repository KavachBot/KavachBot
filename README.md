<div align="center">

<img src="https://github.com/KavachBot/KavachBot/blob/main/assets/logo.png?ex=68d16586&is=68d01406&hm=cc2376d2e50133ddaad10dc11ad3aeab484f0638cc4004fdff80a42db295b28c&=&format=webp&quality=lossless" width="100px" />

<br/>

# KAVACH

**Enterprise-Grade Discord Security**

<br/>

<a href="https://discord.com/oauth2/authorize?client_id=860040650124558336"><img src="https://img.shields.io/badge/ADD%20KAVACH-FREE-00C851?style=for-the-badge&labelColor=0d1117&color=00C851" /></a>
&nbsp;
<a href="https://discord.com/oauth2/authorize?client_id=1051442182466314281"><img src="https://img.shields.io/badge/ADD%20KAVACH-PREMIUM-FFD700?style=for-the-badge&labelColor=0d1117&color=FFD700" /></a>
&nbsp;
<a href="https://discord.gg/Kavach"><img src="https://img.shields.io/badge/DISCORD-SUPPORT%20SERVER-5865F2?style=for-the-badge&labelColor=0d1117&color=5865F2" /></a>
&nbsp;
<a href="https://kavachbot.com"><img src="https://img.shields.io/badge/WEBSITE-KAVACHBOT.COM-FF6B35?style=for-the-badge&labelColor=0d1117&color=FF6B35" /></a>

<br/><br/>

<img src="https://github.com/KavachBot/KavachBot/blob/main/assets/banner.png?ex=68d1657b&is=68d013fb&hm=5fd74aca0b88dfc5ac5d106fc8ed53bde931857e43bbb620a206e297c1316a5e&=&format=webp&quality=lossless" width="780px" />

</div>

<br/>

---

## Overview

**Kavach** — meaning *armor* in Sanskrit — is a battle-tested Discord security bot built to defend servers against raids, nuke attempts, unauthorized mass actions, and malicious actors. Available in both a free and premium tier, Kavach scales to protect communities of any size.

---

## Features

<br/>

<table>
<tr>
<td width="50%" valign="top">

**AntiNuke Protection**
Detects and neutralizes mass channel deletions, role modifications, and permission escalations in real time before damage is done.

<br/>

**Anti-Raid System**
Identifies coordinated join floods and automated bot raids. Automatically lockdowns entry and alerts administrators.

<br/>

**Automated Moderation**
Intelligent auto-mod covering spam, invite links, mass mentions, caps abuse, and fully customizable word filters.

</td>
<td width="50%" valign="top">

**Backup & Restore**
Complete server snapshots — channels, roles, permission overwrites — with one-command restore capability.

<br/>

**Verification Gates**
Captcha-based and button-gate entry systems that block bot accounts and unverified users from accessing your server.

<br/>

**Audit Logging**
Granular, timestamped logs for every moderation action, member join/leave, role assignment, and permission change.

</td>
</tr>
</table>

---

## Official Bots

> These are the **only** verified, official Kavach bots. Always confirm the Discord tag before granting permissions.

<br/>

| Bot | Tag | Purpose | Invite |
|---|---|---|---|
| Kavach | `Kavach#4312` | Core security suite — free for all servers | [Add](https://discord.com/oauth2/authorize?client_id=860040650124558336) |
| Kavach Premium | `Kavach Premium#6962` | Full feature suite with priority support | [Add](https://discord.com/oauth2/authorize?client_id=1051442182466314281) |
| Suno | `Suno#5234` | High-quality music playback | [Add](https://discord.com/oauth2/authorize?client_id=1213200797844897833) |

---

## Free vs Premium

<br/>

| Capability | Free | Premium |
|---|:---:|:---:|
| AntiNuke Core | Yes | Yes |
| Anti-Raid Detection | Yes | Yes |
| Basic Auto-Moderation | Yes | Yes |
| Server Backup & Restore | — | Yes |
| Advanced Verification | — | Yes |
| Custom Alert Channels | — | Yes |
| Extended Audit Logs | — | Yes |
| Priority Support | — | Yes |

*Premium features activate instantly upon payment confirmation.*

---

## Getting Started

**1.** Invite Kavach to your server via the invite link above.

**2.** Run the setup command and configure your desired protection level.

**3.** Assign Kavach a high-ranked role so it can act on threats effectively.

**4.** Kavach handles the rest — silently, in the background, always watching.

---

## Contact

| Purpose | Contact |
|---|---|
| General Support | [support@kavachbot.com](mailto:support@kavachbot.com) |
| Business Inquiries | [admin@kavachbot.com](mailto:admin@kavachbot.com) |
| Sales & Premium Plans | [sales@kavachbot.com](mailto:sales@kavachbot.com) |
| Billing & Payments | [billing@kavachbot.com](mailto:billing@kavachbot.com) |
| Partnerships | [partners@kavachbot.com](mailto:partners@kavachbot.com) |
| Legal Matters | [legal@kavachbot.com](mailto:legal@kavachbot.com) |
| Careers | [hr@kavachbot.com](mailto:hr@kavachbot.com) |
| Discord Support | [discord.gg/Kavach](https://discord.gg/Kavach) |

*Email response time: 24 – 48 hours.*

---

## Legal

[Terms of Service](./Terms.md) &nbsp;·&nbsp; [Privacy Policy](./Privacy.md) &nbsp;·&nbsp; [Refund Policy](./Refund.md) &nbsp;·&nbsp; [Shipping Policy](./Shipping.md)

---

## Security Notice

Kavach will **never** ask for your password or personal credentials. Only interact with the bots listed above — verify the Discord tag before granting any permissions. Critical security alerts may be delivered via Direct Message and cannot be disabled.

---

<div align="center">

<sub>© 2025 Kavach. All rights reserved. &nbsp;·&nbsp; Last Updated: 5 September 2025</sub>

</div>
